...
public static class MovementLimiter
{
    ...
    private static void Player_Jump(On.Player.orig_Jump orig, Player self)
    {
        ...
        if (self.jumpBoost <= 0 && CurrentAbilities.JumpBoost > 1)
            self.jumpBoost += 12f * (CurrentAbilities.JumpBoost - 1);
        else
            self.jumpBoost *= CurrentAbilities.JumpBoost;

        if (onPole)
            self.jumpBoost *= CurrentAbilities.PoleJumpBoost;
    }
    ...
    {
        if (CurrentAbilities.CanWallJump)
        {
            orig(self, direction);
    ...
    private static void Player_UpdateBodyMode(On.Player.orig_UpdateBodyMode orig, Player self)
    {
        try
        {
            if (!CurrentAbilities.ClimbVerticalCorridors && self.bodyMode == Player.BodyModeIndex.CorridorClimb && self.input[0].y > 0
    ...
    private static void Player_UpdateAnimation(On.Player.orig_UpdateAnimation orig, Player self)
    {
        try
        {
            if (!CurrentAbilities.ClimbVerticalPoles
    ...
    {
        if (!CurrentAbilities.CanGrabPoles && self.noGrabCounter <= 0)
        ...
        if (jb > self.jumpBoost)
            self.jumpBoost = jb + (self.jumpBoost - jb) * CurrentAbilities.JumpBoostDecrement;
    ...
    private static void Player_Update(On.Player.orig_Update orig, Player self, bool eu)
    {
        try
        {
            if (!CurrentAbilities.CanUseShortcuts && self.enteringShortCut != null && self.Consious
